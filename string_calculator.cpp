...
std::pair<std::string, std::string> StringCalculator::parseDelimiter(const std::string& input) const {
    if (input.rfind("//", 0) == 0) {
...
std::vector<std::string> StringCalculator::split(const std::string& numbers, const std::vector<std::string>& delimiters) const {
    ...
    while (i < numbers.size()) {
        ...
        if (!matched) token.push_back(numbers[i++]);
    }
